export const SYMPTOM_MAPPING = {
  head: [
    "headache",
    "toothache",
    "facial pain",
    "mouth ulcer",
    "mouth pain",
    "mouth dryness",
    "jaw swelling",
    "gum pain",
    "pain in gums",
    "bleeding gums",
    "swollen or red tonsils",
    "diminished vision",
    "double vision",
    "symptoms of eye",
    "pain in eye",
    "abnormal movement of eyelid",
    "foreign body sensation in eye",
    "spots or clouds in vision",
    "eye redness",
    "lacrimation",
    "itchiness of eye",
    "blindness",
    "eye burns or stings",
    "white discharge from eye",
    "mass on eyelid",
    "swollen eye",
    "eyelid swelling",
    "eyelid lesion or rash",
    "bleeding from eye",
    "irregular appearing scalp",
    "ear pain",
    "pus draining from ear",
    "diminished hearing",
    "ringing in ear",
    "plugged feeling in ear",
    "itchy ear(s)",
    "fluid in ear",
    "pulling at ears",
    "redness in ear",
    "bleeding from ear",
  ],
  neck: [
    "neck pain",
    "neck mass",
    "neck swelling",
    "sore throat",
    "hoarse voice",
    "difficulty speaking",
    "throat swelling",
    "difficulty in swallowing",
    "nasal congestion",
    "coryza",
    "nosebleed",
    "sinus congestion",
    "painful sinuses",
  ],
  chest: [
    "shortness of breath",
    "sharp chest pain",
    "chest tightness",
    "palpitations",
    "irregular heartbeat",
    "breathing fast",
    "cough",
    "heartburn",
    "rib pain",
    "congestion in chest",
    "burning chest pain",
    "hurts to breath",
    "difficulty breathing",
    "wheezing",
    "coughing up sputum",
    "hemoptysis",
    "abnormal breathing sounds",
    "apnea",
  ],
  abdomen: [
    "sharp abdominal pain",
    "nausea",
    "vomiting",
    "diarrhea",
    "burning abdominal pain",
    "upper abdominal pain",
    "stomach bloating",
    "changes in stool appearance",
    "blood in stool",
    "melena",
    "constipation",
    "regurgitation",
    "vomiting blood",
  ],
  pelvis: [
    "lower abdominal pain",
    "pelvic pain",
    "suprapubic pain", // MOVED FROM LEGS
    "groin pain",      // MOVED FROM LEGS
    "rectal bleeding", // MOVED FROM ABDOMEN
    "pain of the anus", // MOVED FROM ABDOMEN
    "mass or swelling around the anus", // MOVED FROM ABDOMEN
    "itching of the anus", // MOVED FROM ABDOMEN
    "vaginal itching",
    "vaginal discharge",
    "vaginal pain",
    "vaginal redness",
    "painful urination",
    "involuntary urination",
    "pain during intercourse",
    "frequent urination",
    "blood in urine",
    "unusual color or odor to urine",
    "hot flashes",
    "intermenstrual bleeding",
    "blood clots during menstrual periods",
    "long menstrual periods",
    "heavy menstrual flow",
    "unpredictable menstruation",
    "painful menstruation",
    "frequent menstruation",
    "infertility",
    "retention of urine",
    "low urine output",
    "excessive urination at night",
    "symptoms of bladder",
    "symptoms of prostate",
    "swelling of scrotum",
    "pain in testicles",
    "symptoms of the scrotum and testes",
    "impotence",
    "kidney mass",
    "symptoms of the kidneys",
    "hesitancy",
  ],
  back: [
    "back pain",
    "low back pain",
    "back mass or lump",
    "back cramps or spasms",
    "back stiffness or tightness",
    "side pain",
  ],
  arms: [
    "hand or finger pain",
    "wrist pain",
    "hand or finger swelling",
    "arm pain",
    "wrist swelling",
    "arm stiffness or tightness",
    "arm swelling",
    "hand or finger stiffness or tightness",
    "elbow pain",
    "elbow swelling",
    "shoulder pain",
    "shoulder stiffness or tightness",
    "hand or finger weakness",
    "arm weakness",
    "hand or finger lump or mass",
    "arm lump or mass",
  ],
  legs: [
    "leg pain",
    "hip pain",
    "leg swelling",
    "foot or toe pain",
    "ankle pain",
    "ankle swelling",
    "bones are painful",
    "knee pain",
    "knee swelling",
    "knee stiffness or tightness",
    "leg weakness",
    "hip stiffness or tightness",
    "foot or toe swelling",
    // Removed: "groin pain", "suprapubic pain"
    "lack of growth",
  ],
  general: [
    "anxiety and nervousness",
    "depression",
    "depressive or psychotic symptoms",
    "dizziness", // MOVED FROM HEAD
    "frontal headache", // MOVED FROM HEAD
    "disturbance of memory", // MOVED FROM HEAD
    "insomnia",
    "abnormal involuntary movements",
    "feeling ill",
    "fever",
    "fatigue",
    "weakness",
    "chills",
    "decreased appetite",
    "weight gain",
    "decreased heart rate",
    "increased heart rate",
    "jaundice",
    "skin swelling",
    "lip swelling",
    "abnormal appearing skin",
    "skin lesion",
    "acne or pimples",
    "skin growth",
    "skin moles",
    "itching of skin",
    "skin dryness, peeling, scaliness, or roughness",
    "skin irritation",
    "itchy scalp",
    "warts",
    "skin rash",
    "diaper rash",
    "fainting",
    "hostile behavior",
    "excessive anger",
    "temper problems",
    "abusing alcohol",
    "drug abuse",
    "low self-esteem",
    "hysterical behavior",
    "antisocial behavior",
    "obsessions and compulsions",
    "fears and phobias",
    "delusions or hallucinations",
    "loss of sensation",
    "focal weakness",
    "paresthesia",
    "symptoms of the face",
    "problems with movement",
    "joint pain",
    "cramps and spasms",
    "ache all over",
    "lower body pain",
    "restlessness",
    "sleepiness",
    "sneezing",
    "allergic reaction",
    "flu-like syndrome",
    "sweating",
    "peripheral edema",
    "fluid retention",
    "irritable infant",
    "infant feeding problem",
    "problems during pregnancy",
    "pain during pregnancy",
    "spotting or bleeding during pregnancy",
    "recent pregnancy",
    "uterine contractions",
    "irregular appearing nails",
    "seizures",
  ],
};

export const ALL_SYMPTOMS = [
	"anxiety and nervousness",
	"depression",
	"shortness of breath",
	"depressive or psychotic symptoms",
	"sharp chest pain",
	"dizziness",
	"insomnia",
	"abnormal involuntary movements",
	"chest tightness",
	"palpitations",
	"irregular heartbeat",
	"breathing fast",
	"hoarse voice",
	"sore throat",
	"difficulty speaking",
	"cough",
	"nasal congestion",
	"throat swelling",
	"diminished hearing",
	"difficulty in swallowing",
	"skin swelling",
	"retention of urine",
	"leg pain",
	"hip pain",
	"suprapubic pain",
	"blood in stool",
	"lack of growth",
	"symptoms of the scrotum and testes",
	"swelling of scrotum",
	"pain in testicles",
	"pus draining from ear",
	"jaundice",
	"white discharge from eye",
	"irritable infant",
	"abusing alcohol",
	"fainting",
	"hostile behavior",
	"drug abuse",
	"sharp abdominal pain",
	"feeling ill",
	"vomiting",
	"headache",
	"nausea",
	"diarrhea",
	"vaginal itching",
	"painful urination",
	"involuntary urination",
	"pain during intercourse",
	"frequent urination",
	"lower abdominal pain",
	"vaginal discharge",
	"blood in urine",
	"hot flashes",
	"intermenstrual bleeding",
	"hand or finger pain",
	"wrist pain",
	"hand or finger swelling",
	"arm pain",
	"wrist swelling",
	"arm stiffness or tightness",
	"arm swelling",
	"hand or finger stiffness or tightness",
	"lip swelling",
	"toothache",
	"abnormal appearing skin",
	"skin lesion",
	"acne or pimples",
	"facial pain",
	"mouth ulcer",
	"skin growth",
	"diminished vision",
	"double vision",
	"symptoms of eye",
	"pain in eye",
	"abnormal movement of eyelid",
	"foreign body sensation in eye",
	"irregular appearing scalp",
	"back pain",
	"neck pain",
	"low back pain",
	"pain of the anus",
	"pain during pregnancy",
	"pelvic pain",
	"impotence",
	"vomiting blood",
	"regurgitation",
	"burning abdominal pain",
	"restlessness",
	"wheezing",
	"peripheral edema",
	"neck mass",
	"ear pain",
	"jaw swelling",
	"mouth dryness",
	"neck swelling",
	"knee pain",
	"foot or toe pain",
	"ankle pain",
	"bones are painful",
	"elbow pain",
	"knee swelling",
	"skin moles",
	"weight gain",
	"problems with movement",
	"knee stiffness or tightness",
	"leg swelling",
	"foot or toe swelling",
	"heartburn",
	"infant feeding problem",
	"vaginal pain",
	"vaginal redness",
	"weakness",
	"decreased heart rate",
	"increased heart rate",
	"ringing in ear",
	"plugged feeling in ear",
	"itchy ear(s)",
	"frontal headache",
	"fluid in ear",
	"spots or clouds in vision",
	"eye redness",
	"lacrimation",
	"itchiness of eye",
	"blindness",
	"eye burns or stings",
	"decreased appetite",
	"excessive anger",
	"loss of sensation",
	"focal weakness",
	"symptoms of the face",
	"disturbance of memory",
	"paresthesia",
	"side pain",
	"fever",
	"shoulder pain",
	"shoulder stiffness or tightness",
	"ache all over",
	"lower body pain",
	"problems during pregnancy",
	"spotting or bleeding during pregnancy",
	"cramps and spasms",
	"upper abdominal pain",
	"stomach bloating",
	"changes in stool appearance",
	"unusual color or odor to urine",
	"kidney mass",
	"symptoms of prostate",
	"difficulty breathing",
	"rib pain",
	"joint pain",
	"hand or finger lump or mass",
	"chills",
	"groin pain",
	"fatigue",
	"regurgitation.1",
	"symptoms of the kidneys",
	"melena",
	"coughing up sputum",
	"seizures",
	"delusions or hallucinations",
	"excessive urination at night",
	"bleeding from eye",
	"rectal bleeding",
	"constipation",
	"temper problems",
	"coryza",
	"hemoptysis",
	"allergic reaction",
	"congestion in chest",
	"sleepiness",
	"apnea",
	"abnormal breathing sounds",
	"blood clots during menstrual periods",
	"pulling at ears",
	"gum pain",
	"redness in ear",
	"fluid retention",
	"flu-like syndrome",
	"sinus congestion",
	"painful sinuses",
	"fears and phobias",
	"recent pregnancy",
	"uterine contractions",
	"burning chest pain",
	"back cramps or spasms",
	"back mass or lump",
	"nosebleed",
	"long menstrual periods",
	"heavy menstrual flow",
	"unpredictable menstruation",
	"painful menstruation",
	"infertility",
	"frequent menstruation",
	"sweating",
	"mass on eyelid",
	"swollen eye",
	"eyelid swelling",
	"eyelid lesion or rash",
	"symptoms of bladder",
	"irregular appearing nails",
	"itching of skin",
	"hurts to breath",
	"skin dryness, peeling, scaliness, or roughness",
	"skin irritation",
	"itchy scalp",
	"warts",
	"skin rash",
	"mass or swelling around the anus",
	"ankle swelling",
	"elbow swelling",
	"bleeding from ear",
	"hand or finger weakness",
	"low self-esteem",
	"itching of the anus",
	"swollen or red tonsils",
	"hip stiffness or tightness",
	"mouth pain",
	"arm weakness",
	"obsessions and compulsions",
	"antisocial behavior",
	"sneezing",
	"leg weakness",
	"hysterical behavior",
	"arm lump or mass",
	"bleeding gums",
	"pain in gums",
	"diaper rash",
	"hesitancy",
	"back stiffness or tightness",
	"low urine output"
]